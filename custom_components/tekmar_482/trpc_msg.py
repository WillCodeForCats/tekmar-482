...
#
name_from_serviceID = dict(list(zip(list(service_formats.keys()), 
...
# name_from_serviceID.
#
serviceID_from_name = dict(list(zip(list(name_from_serviceID.values()), list(name_from_serviceID.keys()))))
...
method_formats = {
        ...
        #*********************************************************************
        0x00000107   :   FieldList(
            'NetworkError',
                Int16('error')
            ),
        
        #*********************************************************************
        0x0000010F   :   FieldList(
            'ReportingState',
                Int8('state')
            ),

        #*********************************************************************
        0x00000117   :   FieldList(
            'OutdoorTemperature',
                Int16('temp')
            ),
        
        #*********************************************************************
        0x0000011F   :   FieldList(
            'DeviceAttributes',
                Int16('address'),
                Int16('attributes')
            ),
       ...
       #*********************************************************************
        ...
        #*********************************************************************
        0x0000016F   :   FieldList(
            'SetbackEnable',
                Int8('enable')
            ),

        #*********************************************************************
        0x00000177   :   FieldList(
            'SetbackState',
                Int16('address'),
                Int8('setback')
            ),
...
#
name_from_methodID = dict(list(zip(list(method_formats.keys()), 
        (f.name for f in list(method_formats.values())))))

#*****************************************************************************
# Lookup method names from method IDs.  This is the reverse of
# name_from_methodID.
#
methodID_from_name = dict(list(zip(list(name_from_methodID.values()), list(name_from_methodID.keys()))))
...
class TrpcPacket:
    ...
    def from_rx_packet(pck_str):
        ...
        if p.type != TYPE_TRPC:
            ...
            try:
                trpc.body, trpc.extra = Record.create(method_formats[trpc.header['methodID']], d)
    ...
    def __str__(self):
        ...
        try: service_name = name_from_serviceID[self.header['serviceID']]
        except KeyError: service_name = '0x%04X' % self.header['serviceID']

        service_name = service_name.ljust(16)

        try: method_name = name_from_methodID[self.header['methodID']]
        except KeyError: method_name = '0x%04X' % self.header['methodID']
